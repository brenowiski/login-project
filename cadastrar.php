...
if(isset($_POST['nome_completo']) && isset($_POST['email']) && isset($_POST['senha']) && isset($_POST['repete_senha'])){
    ...
    if(empty($_POST['nome_completo']) || empty($_POST['email']) ||  empty($_POST['senha']) || empty($_POST['termos'])){
            ...
            if(!$usuario){
                ...
                if($sql->execute(array($nome,$email,$senha_cript,$recupera_senha,$token,$codigo_confirmacao,$status,$data_cadastro))){
                    ...
                    if($modo == "producao"){
                        ...
                        $mail->Body    = '<h1>Por favor confirme seu e-mail abaixo:</h1><a 
                        style="background:red; color:white; text-decoration:none; padding:20px; border-radius:10px" 
                        href="https://pumpking.online/login/confirmacao.php?cod_confirm='.$codigo_confirmacao.'">Confirmar E-mail</a><br><br><p>Equipe Login em PHP</p>';
...
<body>
    <form method="post">
        ...
        <div class="input-group">
            <img class="input-icon" src="img/nome-completo.png">            
            <input <?php if(isset($erro_geral) or isset($erro_nome)){echo 'class="erro-input"';} ?> 
            name="nome_completo" type="text" placeholder="Nome Completo" <?php if(isset($_POST['nome_completo'])){ echo "value='".$_POST['nome_completo']."'";}?> required>
        ...
        <div class="input-group">
            ...
            <input type="email" <?php if(isset($erro_geral) or isset($erro_email)){echo 'class="erro-input"';} ?>  
            name="email" placeholder="Seu melhor email" <?php if(isset($_POST['email'])){ echo "value='".$_POST['email']."'";}?> required>
        ...
        <div class="input-group">
            ...
            name="senha" placeholder="Digite uma senha" <?php if(isset($_POST['repete_senha'])){ echo "value='".$_POST['senha']."'";}?> required>
        ...
        <div <?php if(isset($erro_geral) or isset($erro_checkbox)){echo 'class="erro-input"';}else{echo 'class="input-group"';}?> >
           ...
           <label for="termos">Ao se cadastrar você concorda com a nossa <a class="link" href="#">Política de Privacidade</a> e os 
           <a class="link" href="#">Termos de uso</a></label>
        </div>  
        
        <button class="btn-blue" type="submit">Cadastrar</button>       
        <div class="input-link"><a href="index.php">Já tenho cadastro</a></div>  
        <div><br></div>   
